...
# --- 2.1 导入中介变量 (代谢评分) ---
met_score <- read_tsv("HZQ-260121/SRI_metabolomic_signature_scores_260129.tsv")

# 图片要求：SRI_met_score 划分为4分位数 Q1-Q4
# 计算分位点
quantiles <- quantile(met_score$SRI_met_score, probs = seq(0, 1, 0.25), na.rm = TRUE)

# 划分为分类变量
met_score$met_quartile <- cut(met_score$SRI_met_score, 
                              breaks = quantiles, 
                              labels = c("Q1", "Q2", "Q3", "Q4"), 
                              include.lowest = TRUE)

# 图片要求：Q4作为ref (参考水平)
met_score$met_quartile <- relevel(met_score$met_quartile, ref = "Q4")

# 仅保留需要的变量 (eid, 原始分, 分组变量)
met_score <- met_score %>%
  dplyr::select(eid, SRI_met_score, met_quartile)
...
# --- 2.4 合并固定底表 ---
df_base_fixed <- met_score %>%
  inner_join(GGIR_selected, by = "eid") %>%
  inner_join(CovariatesImputed, by = "eid")

print(paste("固定底表人数:", nrow(df_base_fixed))) #89436
