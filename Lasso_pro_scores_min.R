...
##### 1. 设置文件路径 导入数据#####
setwd("D:/ukb data/Sleep regularity/Sleep regularity/Pro_lasso")

## 1）读取线性回归结果并筛选蛋白质 (新增步骤)
...
# 读取 "Proteo_Model2" sheet (对应 Model 2 的结果)
df_lm_res <- read.xlsx(lm_result_file, sheet = "Proteo_Model2")

# 筛选 FDR < 0.05 的显著代谢物
significant_pros <- df_lm_res %>%
  filter(FDR < 0.05) %>%
  pull(Outcome) # 提取代谢物名称列

cat("Model 2 中 FDR < 0.05 的代谢物数量:", length(significant_pros), "\n")

if(length(significant_pros) == 0) stop("没有显著的代谢物，无法进行 LASSO 分析！")
...
pro.vars <- intersect(significant_pros, colnames(df_pro))

length(pro.vars) #记录数量 met186 pro2920

# 合并
df_pro_subset <- df_pro %>% select(all_of(c("eid", pro.vars)))


## 4）合并
df_merge <- GGIR_selected %>%
  inner_join(df_pro_subset, by = "eid") %>%
...
## 动态计算图片高度
# 统计代谢物个数
n_pros <- nrow(coef_df_pro)

# 设定高度策略：
# 基础高度 2 英寸 (给标题和坐标轴留空)
# 每个代谢物分配 0.15 英寸的高度 (可以根据需要调整这个系数，0.15-0.2 比较合适)
# 设置最小高度为 5，防止只有几个代谢物时图太扁
plot_height <- max(4, 1 + n_pros * 0.1)

cat("检测到", n_pros, "个代谢物，将图片高度设定为:", plot_height, "英寸\n")

#作图
coef_plot <- ggplot(
  coef_df_pro, 
  aes(
    x = reorder(protein, beta), 
    y = beta, 
    fill = beta
  )
) +
  ...
  labs(
    title = "Selected proteins and Coefficients", 
    x = "Protein", 
    y = "Coefficient Value"
  ) +
  theme_minimal() +
...
ggsave(
  filename = "SRI_proteomic_signature_coefficients.pdf",
  plot = coef_plot,
  width = 7,
...
ggsave(
  filename = "SRI_pro_lasso_r2_cvHoldOuts.pdf",
  device = "pdf",
  height = 4,
  width = 6
)
...
fwrite(
  ...
  file = "SRI_pro_lasso_cv_R2_values.tsv",
  sep = "\t"
)